...
class ActivationTokenModel(Base):
    ...
    __table_args__ = (UniqueConstraint("user_id", name="unique_activation_user"),)

    @classmethod
    def create(cls, user_id: int, hours_valid: int = 24) -> "ActivationTokenModel":
        return cls(
            user_id=user_id,
...
class PasswordResetTokenModel(Base):
    ...
    __table_args__ = (UniqueConstraint("user_id", name="unique_password_reset_user"),)

    @classmethod
    def create(cls, user_id: int, hours_valid: int = 1) -> "PasswordResetTokenModel":
        return cls(
            user_id=user_id,
            token=generate_secure_token(),
            expires_at=datetime.now(timezone.utc) + timedelta(hours=hours_valid),
